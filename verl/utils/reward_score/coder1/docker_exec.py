...
def code_exec_docker(code, stdin: str = None, timeout=_DEFAULT_TIMEOUT_SECONDS, pytest: str = None):
    ...
    if pytest:
        if stdin is not None:
            return False, _ERROR_MSG_PREFIX + "STDIN is not supported with pytest"
        tmpdir = tempfile.TemporaryDirectory()
        container_kwargs["volumes"] = {tmpdir.name: {"bind": "/tmp"}}
        script_path = Path(tmpdir.name) / "solution.py"
        script_path.write_text(code)
        test_path = Path(tmpdir.name) / "test_solution.py"
        test_path.write_text(pytest)
        pycmd = ["python", "-m", "pytest", "/tmp"]
    elif len(code) < CLI_ARG_SIZE_LIMIT:
        pycmd = ["python", "-c", code]
    else:
        tmpdir = tempfile.TemporaryDirectory()
        container_kwargs["volumes"] = {tmpdir.name: {"bind": "/tmp"}}
    ...
    if stdin:
        ...
        if len(stdin) < CLI_ARG_SIZE_LIMIT:
            ...
            if tmpdir is None:
                ...
                container_kwargs.setdefault("volumes", {})[tmpdir.name] = {"bind": "/tmp"}
