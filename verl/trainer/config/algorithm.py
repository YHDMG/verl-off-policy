...
__all__ = ["AlgoConfig", "FilterGroupsConfig", "KLControlConfig", "OffPolicyConfig", "RolloutCorrectionConfig"]
...
class OffPolicyConfig(BaseConfig):
    """Configuration for synchronous off-policy replay on top of PPO/GRPO.

    This v1 implementation is intentionally scoped to actor-only training in the
    synchronous ``RayPPOTrainer`` path. Samples are collected online from past
    rollout batches, stored in a replay buffer, and later replayed alongside the
    current on-policy batch during actor updates.
    """
...
class AlgoConfig(BaseConfig):
    ...
    # Off-policy replay for the synchronous PPO trainer. Intended for actor-only v1 usage.
    off_policy: Optional[OffPolicyConfig] = None
